...
export class NbTimePickerDirective implements AfterViewInit {
  ...
  protected subscribeOnApplyClick() {
    this.timepicker.onSelectTime.subscribe((value: NbSelectedTimePayload) => {
      if (this.timepicker._useFullTimeFormat && value.time.fullTime) {
  ...
  protected handleInputChange(value: string) {
    const isValidDate: boolean = this.dateService.isValidDateString(value, this._timePickerComponent.timeFormat);

    if (isValidDate) {
      const date: Date = this.dateService.parse(value, this._timePickerComponent.timeFormat);
      this._timePickerComponent.selectedTime = {
        hour: this.formatToString(this._timePickerComponent._isTwelveHoursFormat && value !== '12' ?
        ...
        ampm: this.nbCalendarTimeModelService.getAmPm(date, this._timePickerComponent.timeFormat),
        fullTime: this._timePickerComponent._useFullTimeFormat ?
          this.dateService.format(date, this._timePickerComponent.timeFormat) : '',
      };
    }
  }
